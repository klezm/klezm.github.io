...
- **Starlight Integration**: This project uses `@astrojs/starlight` for blog features.
...
- Use built-in loaders: `docsLoader()` for Starlight, `glob()` for file-based content, `file()` for single files
...
const docs = defineCollection({
  loader: docsLoader(),
  schema: docsSchema({
    extend: (context) => z.object({ /* custom fields */ }),
  }),
});
```
...
- **Tailwind CSS v4**: Use `@tailwindcss/vite` for styling. Utility classes are preferred.
