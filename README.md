...
Only `title` and `date` are required. `date` is also what separates a real post
from the tag and author pages generated under the same `blog/` prefix, so code
that asks "is this a post?" checks for it.

`src/content/docs/blog/kitchen-sink.mdx` demonstrates every supported feature
and is the fastest way to see what's available: KaTeX math, code blocks,
Starlight asides and GitHub-style alerts, custom `:::idea` / `:::proof` blocks,
footnotes with hover popovers, BibTeX citations, keyboard keys and zoomable
images.

### Citations

Point `bibliography` at a `.bib` file relative to the post and cite with
`[@citekey]`. Two constraints, because each one aborts the whole build: the
`.bib` file may not contain `%` comments, and may not use `\url{}` macros — use
a plain `url = {…}` field instead.

## How it fits together

The site is blog-only. There is no sidebar anywhere, enforced in
`src/middleware/sidebar.ts` rather than with CSS.

- **`astro.config.mjs`** — all plugin wiring. Several options are load-bearing
  and annotated with why; read the comments before changing them.
...
- **`src/plugins/rehype-citation-frontmatter.js`** — makes `rehype-citation`
  read its bibliography path from post frontmatter instead of one global
  option.
- **`src/styles/katex-theme.css`** — light-mode colour corrections for
  equations that were coloured for a dark background.
